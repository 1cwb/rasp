...
namespace rasp
{
    struct Slice;
    struct Buffer;
    int LinCodec::tryDecode(Slice data, Slice& msg)
    {
    ...
    void LinCodec::encode(Slice msg, Buffer& buff)
    {
        buff.append(msg).append("\r\n");
    }
    CodecBase* LinCodec::clone()
    {
        return new LinCodec();
    }
    ...
    void LengthCodec::encode(Slice msg, Buffer& buff)
    {
        buff.append("mBdT").appendValue(net::hton((int32_t)msg.size())).append(msg);
    }
