...
namespace rasp
{
    ...
    Logger::Logger() : level_(LINFO), lastRotate_(time(nullptr)), rotateInterval_(86400)
    {
        tzset();
        fd_ = -1;
    ...
    {
        int fd = open(filename.c_str(), O_APPEND | O_CREAT | O_WRONLY | O_CLOEXEC, DEFFILEMODE/*0666*/);
        if(fd < 0)
        {
    ...
    void Logger::setLogLevel(const std::string& level)
    {
        ...
        size_t num = sizeof(levelStrs_)/sizeof(const char*);
